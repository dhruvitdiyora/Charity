...
namespace CharityAPI.Controllers
{
    ...
    public class UrgencyController : ControllerBase
    {
        ...
        public ActionResult<Urgency> AddUrgrncy(Urgency urgency)
        {
        ...
        [HttpPut("{UrgencyId}")]
        //[Authorize(Roles = UserRoles.Admin)]
        public IActionResult EditUrgency(int UrgencyId, Urgency urgency)
        {
            if (!ModelState.IsValid)
                return BadRequest("Not a valid model");
            var stateObject = _urgencyServices.GetUrgencyByUrgencyId(UrgencyId);
            if (stateObject != null)
            {
                _urgencyServices.EditUrgency(urgency);
                return Ok();
            }
            return NotFound();
        }
